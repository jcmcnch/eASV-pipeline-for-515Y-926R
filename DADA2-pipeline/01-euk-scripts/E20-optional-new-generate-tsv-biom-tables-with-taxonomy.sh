...
mkdir 20-exports/02-taxonomy-tsv/SILVA132

mkdir 20-exports/03-biom-tables-with-tax/
mkdir 20-exports/03-biom-tables-with-tax/PR2
mkdir 20-exports/03-biom-tables-with-tax/SILVA132
...
for item in `ls 19-reclassified/SILVA132/*.qza`
        do
        filestem=`basename $item -classification.qza`
        qiime tools export --input-path $item --output-path 20-exports/02-taxonomy-tsv/SILVA132
        sed -i '1c#OTUID\ttaxonomy\tconfidence' 20-exports/02-taxonomy-tsv/SILVA132/taxonomy.tsv
        mv 20-exports/02-taxonomy-tsv/SILVA132/taxonomy.tsv 20-exports/02-taxonomy-tsv/SILVA132/$filestem\-taxonomy.tsv
done

#Part 2: Add the taxonomy.tsv information as "metadata" to the .biom table.

for item in `ls 20-exports/02-taxonomy-tsv/SILVA132/*.tsv`
        do
        filestem=`basename $item .tsv`
        biom add-metadata -i 20-exports/01-biom-table/All-18S-table.biom -o 20-exports/03-biom-tables-with-tax/SILVA132/$filestem.biom --observation-metadata-fp $item --sc-separated taxonomy
done

#Part 3: Convert the final .biom table into a .tsv file.

for item in `ls 20-exports/03-biom-tables-with-tax/SILVA132/*.biom`
        do
        filestem=`basename $item -taxonomy.biom`
        biom convert -i $item -o 20-exports/04-converted-biom-to-tsv/SILVA132/$filestem\-converted-biom.tsv --to-tsv --header-key taxonomy
done
