...
#remove empty files
find ./results/01-split/*euk* -size 0 -print0 | xargs -0 rm -- 2> /dev/null

#the bit you want to cut from the file names, leaving only the sample name
cutME=".euk.R1.fastq.gz" 
...
for item in ${snakemake_input[0]}; do
	...
	R1=`ls results/01-split/$sampleIDstring.euk.R1.fastq.gz`
	R2=`ls results/01-split/$sampleIDstring.euk.R2.fastq.gz`
